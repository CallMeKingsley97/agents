...
- 默认只开发本地 SQL，不自动部署 DataWorks；部署属于共享环境变更，必须单独确认。
...
## 第六步：DataWorks 部署（可选，需用户确认）

SQL 自检通过后，向用户询问是否需要部署到 DataWorks。部署步骤复用 `/alibabacloud-dataworks-datastudio-develop` skill。
...
| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| **工作流** | `bg_ads_ai` | 所有新节点默认放到此工作流下 |
| **节点名称** | 取 SQL 文件名（去 `.sql`） | 如 `ads_xxx.sql` → 节点名 `ads_xxx` |
| **调度时间** | 每天 06:00 | cron: `00 00 06 * * ?`，时区 `Asia/Shanghai` |
| **上游依赖** | 仅根节点 | `depends.output` = `${projectIdentifier}_root` |
| **参数** | `dt=$[yyyy-mm-dd-1]` | 节点参数，SQL 中用 `${dt}` 引用 |
| **节点类型** | `ODPS_SQL` | MaxCompute SQL 节点 |

### 强制动作

1. **检查权限**: 先确认当前账号在 DataWorks 项目中有"数据开发-读写"权限（开发/空间管理员/项目所有者角色）。如无权限，直接告知用户并终止部署
2. **检查工作流**: `list-workflow-definitions` 检查 `bg_ads_ai` 是否存在；不存在则 `create-workflow-definition` 创建
3. **检查节点名冲突**: `list-nodes --name <节点名>` 检查是否已存在同名节点
4. **创建节点**: 按 `/alibabacloud-dataworks-datastudio-develop` skill 的流程创建本地文件结构 → build → validate → `create-node --container-id <WorkflowId>`
5. **设置调度**: `update-workflow-definition` 为工作流设置 `trigger`
6. **发布上线**: `create-pipeline-run --type Online --object-ids <WorkflowId>` → 轮询 → 逐阶段推进，直到 `Pipeline.Status == 'Success'`
7. **结果汇报**: 输出以下信息给用户确认

| 项目 | 值 |
|------|-----|
| DataWorks 项目 | `datawarehouse` (ID: 2402) |
| 工作流 | `bg_ads_ai` |
| 节点名称 | `{node_name}` |
| 节点类型 | ODPS_SQL |
| 调度时间 | 每天 06:00 |
| 参数 | `dt=$[yyyy-mm-dd-1]` |
| 依赖 | `${projectIdentifier}_root` |
| 发布状态 | Success / 失败原因 |
...
- [ ] 已检查工作流是否存在（不存在则创建）
- [ ] 已检查节点名是否冲突
- [ ] 已调用 `/alibabacloud-dataworks-datastudio-develop` skill 完成创建和发布
- [ ] Pipeline 最终状态为 `Success`（或已向用户说明失败原因）
