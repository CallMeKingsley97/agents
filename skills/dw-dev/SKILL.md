...
### 第二步：读取输入表结构
1. 首先加载 /alibabacloud-dataworks-metadata skill, 通过该skill搜索输入表的结构，如果查询成功，跳转步骤4；如果skill不可用或者搜索不到输入表，再继续下面的2、3步骤。
2. 查找用户提供的输入表文件、逻辑可复用文件（在当前项目目录中搜索相关的 `.sql` / `.md` / `.csv` 文件）
3. 如果用户直接粘贴了表结构，解析其中的表名、字段、类型、注释
4. 梳理输入表之间的关系（主外键、业务关联逻辑）
5. 读取逻辑可复用文件，找到与需求中字段名称一致的字段，梳理这些字段的来源逻辑，后续可以将这些复用逻辑写入方案设计文档中
6. 输出《输入表梳理》，包含：
